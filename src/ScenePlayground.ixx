...
export class ScenePlayground : public IScene
{
public:
...
private:
	static constexpr std::string_view BackgroundImage = "playground.png";
...
private:
	...
	Polygon2d m_bounds;
	Polygon2d m_forest_path_bounds;
	Polygon2d m_playground2_bounds;
	EditorGrid m_grid;
	Dog m_dog;
	Baby m_baby;
	...
	, m_camera2d{ render_context.ShouldFlipScreenY() }
{
	...
	m_forest_path_bounds.SetVertices({
		{-2.0f, -4.0f},
		{-1.75f, -4.0f},
		{-1.75f, -2.0f},
		{-2.0f, -2.0f},
	});
	m_playground2_bounds.SetVertices({
		{1.75f, -4.0f},
		{2.0f, -4.0f},
		{2.0f, -2.0f},
		{1.75f, -2.0f},
...
// override
void ScenePlayground::OnWindowResized(int width, int height)
{
...
// override
std::optional<SceneTransition> ScenePlayground::Update(float dt, Input const & input)
{
	...
	if (m_scene_state == SceneState::Gameplay)
	{
		...
		if (m_forest_path_bounds.Contains(dog_pos))
			return SceneTransition{ SceneId::ForestPath };
		if (m_playground2_bounds.Contains(dog_pos))
			return SceneTransition{ SceneId::Playground2 };
	}
...
// override
void ScenePlayground::DestroyPendingAssets() const
{
	m_asset_manager.DestroyPendingAssets();
}

// override
void ScenePlayground::Render() const
{
	m_fps_label.RenderOffscreenTexture();
	m_renderer.Render();
}

void ScenePlayground::ChangeSceneState(SceneState new_state)
{
