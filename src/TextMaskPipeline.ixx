...
export module TextMaskPipeline;
...
private:
	TextMaskPipeline() = delete;
};

std::expected<dh::Pipeline, dh::GraphicsError> TextMaskPipeline::CreatePipeline(
	dh::RenderContext const & render_context,
	std::filesystem::path const & shaders_path,
	Camera2d const & camera2d,
	AssetManager const & asset_manager)
{
	...
	builder.SetPerObjectConstantsCallback(
		[&asset_manager](dh::Pipeline const & pipeline, void const * object_data)
		{
			if (!object_data)
			{
				LOG_FIRST_N(ERROR, 10) << "TextMaskPipeline: ObjectData is null";
				return;
			}
			...
			{
				LOG_FIRST_N(ERROR, 10) << "TextMaskPipeline: No texture found in pool for AssetId: " << data->tex_id.GetIndex();
			}
